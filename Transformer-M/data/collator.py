...
def collator_3d_moleculeace(items, max_node=512, multi_hop_max_dist=20, spatial_pos_max=20):
    # 1. 过滤无效样本和超大分子 (此时 items 还是 Data 对象列表)
    # items = [item for item in items if item is not None and item.x.size(0) <= max_node]
    items = [item for item in items if item is not None]##过滤超大分子会产生不一致，Fairseq 图与标签维度异步
    if len(items) == 0:
        return None
    ...
    for item in items:
        n_node = item.x.size(0)
        # --- [修复 1] 动态伪造 pos 防止 NoneType 报错 ---
        pos = getattr(item, 'pos', None)
        if pos is None:
            # 伪造一个全 0 的坐标系，只为满足原生代码格式，不参与实际计算
            pos = torch.zeros((n_node, 3), dtype=torch.float32)
        ...
        processed_items.append({
            ...
            'y_mean': item.y_mean,
            'y_std': item.y_std,
            'is_cliff': getattr(item, 'is_cliff', None) # [修复 3] 必须保留活性悬崖标签！
        })
    ...
    pos_stds_list = [it['pos_std'] for it in processed_items]
    masks = [it['mask'] for it in processed_items]
    is_cliffs = [it['is_cliff'] for it in processed_items if it['is_cliff'] is not None]
    ...
    is_cliff = torch.cat(is_cliffs) if len(is_cliffs) > 0 else None

    # --- [修复 2] 3D 统计量的安全 Float 填充 ---
    # 使用 F.pad 在矩阵的右侧和下侧补 0.0，防止 Attention 爆炸
    if pos_means_list[0] is not None:
        pos_mean_padded = []
        pos_std_padded = []
        for pm, ps in zip(pos_means_list, pos_stds_list):
            n = pm.size(0)
            # F.pad 参数格式 (left, right, top, bottom)
            # 我们向右补齐 (max_node_num - n) 列，向下补齐 (max_node_num - n) 行
            p_m = F.pad(pm, (0, max_node_num - n, 0, max_node_num - n), value=0.0).unsqueeze(0)
            p_s = F.pad(ps, (0, max_node_num - n, 0, max_node_num - n), value=0.0).unsqueeze(0)
            pos_mean_padded.append(p_m)
            pos_std_padded.append(p_s)
        
        pos_mean = torch.cat(pos_mean_padded) # [Batch, max_node_num, max_node_num]
        pos_std = torch.cat(pos_std_padded)
    else:
        pos_mean, pos_std = None, None
