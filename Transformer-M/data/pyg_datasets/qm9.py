...
class newHQM9(InMemoryDataset):
    ...
    def __init__(self, root, transform=None, pre_transform=None,
        ...
        self.data, self.slices, self.extra_pos_mean, self.extra_pos_std, self.extra_valid= torch.load(self.processed_paths[0])
        self.train_mean = 0.0
        self.train_std = 0.0
    ...
    def process(self):
        ...
        for i, mol in enumerate(tqdm(suppl)):
            ...
            # pos
            # === 核心修改：生成 RDKit 粗构象 (Rough Conformation) ===
            # 1. 复制一个分子对象用于生成，避免干扰原分子属性
            m_rdkit = Chem.Mol(mol) 
            m_rdkit = Chem.AddHs(m_rdkit) # 必须加氢，否则构象不物理
            
            # 使用 ETKDG 算法快速生成单个构象
            ps = AllChem.ETKDGv3()
            ps.randomSeed = 42 # 固定种子保证可复现
            res = AllChem.EmbedMolecule(m_rdkit, ps)
            
            if res >= 0:
                # 如果成功，进行简单的力场优化提高质量
                AllChem.MMFFOptimizeMolecule(m_rdkit)
                m_rdkit = Chem.RemoveHs(m_rdkit) # 移除氢以匹配原有节点数
                conf_rdkit = m_rdkit.GetConformer()
                # 替换原来的 pos 为 RDKit 生成的坐标
                pos = torch.tensor(conf_rdkit.GetPositions(), dtype=torch.float32)
            else:
                # 如果生成失败，fallback 到 SDF 原有的坐标
                print(f"Warning: RDKit failed for mol {i}, using original SDF pos.")
                conf = mol.GetConformer()
