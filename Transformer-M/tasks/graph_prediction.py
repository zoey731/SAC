...
class GraphPredictionConfig(FairseqDataclass):
    ...
    loss_type: str = field(
        default="BCE",
        metadata={
            "help": "loss type, L1, L2, or BCE (for classification)"
        }
    )
...
class GraphPredictionTask(FairseqTask):
    ...
    def __init__(self, cfg):
        super().__init__(cfg)
        if self.cfg.dataset_name.startswith("mol"):##这里分辨一下到底是哪个类
            self.dm = PYGPreprocessedData(dataset_name=self.cfg.dataset_name, dataset_path=self.cfg.data_path, seed=self.cfg.seed)
        elif self.cfg.dataset_name.startswith("ogbg"):
            self.dm = OGBPreprocessedData(dataset_name=self.cfg.dataset_name, dataset_path=self.cfg.data_path, seed=self.cfg.seed)
    ...
    def load_dataset(self, split, combine=False, **kwargs):
        ...
        if split == "train":
            ...
            # --- 增加诊断打印 ---
            print(f"DEBUG: 原始验证集样本数: {len(batched_data)}")
            if len(batched_data) > 0:
                print(f"DEBUG: 验证集第一个样本节点数: {batched_data[0].x.size(0)}")
        elif split == "test":
