...
class TransformerMModel(FairseqEncoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--no-2d", action='store_true', default=False, help="remove 2D encodings"
        )
        parser.add_argument(
...
class RegressionHead(nn.Module):
    def __init__(self, input_dim, inner_dim, activation_fn, dropout=0.1):
        super().__init__()
        self.dense = nn.Linear(input_dim, inner_dim)
        self.activation_fn = utils.get_activation_fn(activation_fn)
...
def base_architecture(args):
    ...
    args.load_qm9 = getattr(args, "load_qm9", False)
