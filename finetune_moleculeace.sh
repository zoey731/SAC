...
if [[ $task == "hiv" || $task == "bace" || $task == "bbbp" || $task == "clintox" || $task == "sider" || $task == "tox21" || $task == "toxcast" ]]; then
  loss="BCE"
else
  loss="L2"
fi
...
[ -z "${lr}" ] && lr=1e-4
# [ -z "${lr}" ] && lr=1e-5
[ -z "${end_lr}" ] && end_lr=1e-5
# [ -z "${warmup_steps}" ] && warmup_steps=60000
[ -z "${warmup_steps}" ] && warmup_steps=2000
[ -z "${total_steps}" ] && total_steps=12000
[ -z "${layers}" ] && layers=12
...
[ -z "${weight_decay}" ] && weight_decay=1e-5
[ -z "${sandwich_ln}" ] && sandwich_ln="false"
[ -z "${droppath_prob}" ] && droppath_prob=0.0

[ -z "${loss_type}" ] && loss_type=L2

# [ -z "${std_type}" ] && std_type="std_logits"  ##这里对比一下qm9的脚本，这里并未传入这个参数
[ -z "${readout_type}" ] && readout_type="cls"

[ -z "${save_interval}" ] && save_interval=1

[ -z "${task_idx}" ] && task_idx=CHEMBL204_Ki
[ -z "${dataset_name}" ] && dataset_name="moleculeace-${task_idx}"
...
[ -z "${MASTER_PORT}" ] && MASTER_PORT=10001
[ -z "${OMPI_COMM_WORLD_SIZE}" ] && OMPI_COMM_WORLD_SIZE=1

[ -z "${save_prefix}" ] && save_prefix='exp'
[ -z "${ckpt_path}" ] && ckpt_path='/home/zhaoqc/Transformer_bias_add_moleculeace/L12-old.pt'
...
python -m torch.distributed.launch --use_env --nproc_per_node=$n_gpu --master_port=$MASTER_PORT $ddp_options train.py \
	...
	--task graph_prediction --criterion graph_prediction --arch transformer_m --num-classes 1 \
	--lr $lr   --end-learning-rate $end_lr --lr-scheduler polynomial_decay \
  --power 1 \
  ...
  --loss-type $loss_type  \
  --sentence-avg  \
	--tensorboard-logdir $tsb_dir --save-dir $save_dir  $load_pretrain_args --no-epoch-checkpoints --save-interval $save_interval | tee $save_dir/train_log.txt
