...
def _upgrade_state_dict(state):
    ...
    if "optimizer_history" not in state:
        state["optimizer_history"] = [
            {"criterion_name": "CrossEntropyCriterion", "best_loss": state.get("best_loss", float("inf"))}
        ]
    ...
    if "epoch" not in state["extra_state"]:
        state["extra_state"]["epoch"] = 1
    if "iterations_in_epoch" not in state["extra_state"]:
        state["extra_state"]["iterations_in_epoch"] = 0
