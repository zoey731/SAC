...
[ -z "${total_steps}" ] && total_steps=20000
[ -z "${layers}" ] && layers=12
...
[ -z "${dataset_name}" ] && dataset_name="molnet-esol"

[ -z "${task_idx}" ] && task_idx=esol
...
[ -z "${MASTER_PORT}" ] && MASTER_PORT=10030
[ -z "${OMPI_COMM_WORLD_SIZE}" ] && OMPI_COMM_WORLD_SIZE=1

[ -z "${save_prefix}" ] && save_prefix='exp'
[ -z "${ckpt_path}" ] && ckpt_path='/home/duanjw/workspace/OStars/ZJ/Transformer_bias_add_moleculenet/L12-old.pt'
